...
public class excelutills implements constants {
	...
	public static String getDataFromCell(String path, String sheet, int row) throws Throwable
	{
		 fis = new FileInputStream(excelPath);
		 wb = WorkbookFactory.create(fis);
		 sh = wb.getSheet(sheet);
		 r = sh.getRow(row);
		String cellValue = c.getStringCellValue().toString();
		return cellValue;
	}
